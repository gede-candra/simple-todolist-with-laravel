...
@section('content')
    <h2 class="mt-3">Kategori Todo</h2>
    <hr>
    ...
    <div class="shadow mt-2 p-3">
        @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Creator')
            <a href="{{ route('kategori-todo.create') }}" class="btn btn-danger mb-3">Tambah Data </a>
        @endif
        <table class="table table-bordered">
            <thead class="table-danger">
                <tr>
                    ...
                    @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Editor' || auth()->user()->role == 'Publisher')
            ...
            <tbody>
                @foreach ($todoCategories as $todoCategory)
                    <tr>
                        ...
                        @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Editor' || auth()->user()->role == 'Publisher')
                            <td>
                                <div class="d-flex justify-content-between align-items-center gap-1" style="width: fit-content;">
                                    @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Editor')
                                        <a href="{{ route('kategori-todo.edit', $todoCategory) }}" class="btn">
                                    ...
                                    @if (auth()->user()->role == 'Admin' || auth()->user()->role == 'Publisher')
                                        <form action="{{ route('kategori-todo.destroy', ['kategori_todo' => $todoCategory]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn">
                                                <svg width="25px" height="25px" viewBox="0 -0.5 21 21" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                                    <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                        <g id="Dribbble-Light-Preview" transform="translate(-179.000000, -360.000000)" fill="#dc3545">
